  ...
  const filterReducer = (state, action) => {
    switch (action.type) {
      case "LOAD_FILTER_PRODUCTS":
        // Ensure the payload contains price data and map over it
        let priceArr = action.payload.map((curElem) => curElem.price || 0); // Default price to 0 if missing
        ...
        // Check if priceArr is empty and handle accordingly
        let maxPrice = priceArr.length > 0 ? Math.max(...priceArr) : 0;
        console.log("🚀 ~ file: filterReducer.js ~ line 23 ~ maxPrice", maxPrice);
